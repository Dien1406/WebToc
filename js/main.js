...
function KommunicateChat() {
  document.addEventListener("DOMContentLoaded", function () {
    var kommunicateSettings = {
      appId: "34c478b7f529a90a5eb1bf7c20b2f25e",
      popupWidget: true,
  ...
  if (temp[1] == "" || temp[1] == undefined || temp[1].search("all") == 0) {
    ...
    for (var i = vitri; i < productArray.length; i++) {
      ...
      <div class="product-item">
            ...
            <button class="icon-btn" onClick="addCart(${
              productArray[i].id
            })"><i class="fa-sharp fa-solid fa-cart-plus"></i></button>
            <button class="buy-now" id="bn${i}" onclick="showProductInfo('${
        productArray[i].id
      }')">MUA NGAY</button>
      </div>
    </li>`;
    ...
    for (var i = 0; i < productArray.length; i++) {
      if (productArray[i].type == typeProduct) arrtempt.push(productArray[i]);
    }
  ...
  for (var i = 0; i < productArray.length; i++) {
    if (productArray[i].id == productid) {
      ...
      document.getElementById("productbrand").innerHTML =
        "Thương hiệu: " + productArray[i].brand;
      document.getElementById("productprice").innerHTML =
        "Giá: " + currency(productArray[i].price);
      document.getElementById("productdetail").innerHTML =
        productArray[i].detail;
      document.getElementById("imgbig").src = productArray[i].img;
      document
        .querySelector("#info .right button.addtocart")
        .setAttribute("onclick", "addCart(" + productid + ")");
    }
  }
}
...
function createUser() {
  ...
  // Nếu có bất kỳ xác thực nào không thành công, trả về false để ngăn việc submit form
  if (flag == false) {
    return false;
  }

  // Nếu tất cả các xác thực đều thành công, tạo một đối tượng người dùng mới
  var d = new Date();
  var datesignup =
  ...
  // Lấy dữ liệu người dùng hiện tại từ local storage hoặc khởi tạo một mảng rỗng nếu chưa có dữ liệu
  var userArray = localStorage.getItem("user")
  ...
  userArray.push(user);
  // Lưu trạng thái mới của mảng người dùng vào local storage
  localStorage.setItem("user", JSON.stringify(userArray));
  customAlert("Bạn đã đăng ký thành công!");
  // Gọi các hàm cập nhật giao diện người dùng (không được cung cấp trong đoạn mã này)
  showUser();
  showLogin();
}
function login() {
  ...
  // Lấy dữ liệu người dùng từ local storage, nếu không có thì khởi tạo một mảng rỗng
  var userArray = localStorage.getItem("user")
    ? JSON.parse(localStorage.getItem("user"))
    : [];
  for (var i = 0; i < userArray.length; i++) {
    if (username == userArray[i].username) {
      if (password == userArray[i].password) {
        // Nếu thông tin đăng nhập đúng, đóng form đăng nhập, lưu thông tin người dùng đăng nhập vào local storage và làm mới trang
        closeform();
...
function checklogin() {
  ...
  <i class="fa-solid fa-right-from-bracket"></i>
      ...
      "<li><button>" +
        user.fullname +
        '</button><button id="btnlogout" onClick="logout(\'index.html\')">LOGOUT <i class="fa-solid fa-right-from-bracket"></i></button></li>' +
        '<li><button onClick="location.href=\'file/cart.html\'"><i class="fa-solid fa-cart-shopping"></i></button></li>';
    }
    document.querySelector("#topbar #header__navbar ul.nav-right").innerHTML =
      s;
  }
}
...
function search() {
  //Khởi tạo một mảng tạm thời để lưu trữ các sản phẩm được tìm thấy.
  let tmparr = [];
  //Lấy giá trị của ô tìm kiếm (search-text) và chuyển đổi thành chữ thường để so sánh dễ dàng hơn.
  var productsearch = document
    .getElementById("search-text")
    .value.toLowerCase();
  //Lấy danh sách sản phẩm từ local storage và chuyển đổi thành một mảng JavaScript.
  var productArray = JSON.parse(localStorage.getItem("product"));
  var s = "";
  if (document.getElementById("searchextend").className == "") {
    for (var i = 0; i < productArray.length; i++) {
      if (
        (productArray[i].name.toLowerCase().search(productsearch) != -1 ||
          productArray[i].type.toLowerCase().search(productsearch) != -1) &&
        productsearch != ""
      ) {
    ...
    if (typesearch == "all") {
      for (var i = 0; i < productArray.length; i++) {
        if (
          productArray[i].name.toLowerCase().search(productsearch) != -1 &&
          parseInt(productArray[i].price) >= parseInt(pricefrom) &&
          parseInt(productArray[i].price) <= parseInt(priceto)
        ) {
          tmparr.push(productArray[i]);
        }
      }
      ...
      for (var i = 0; i < productArray.length; i++) {
        if (
          productArray[i].name.toLowerCase().search(productsearch) != -1 &&
          productArray[i].type.toLowerCase().trim() ==
            typesearch.toLowerCase().trim()
        ) {
          tmparr.push(productArray[i]);
        }
      }
      ...
      for (var i = 0; i < productArray.length; i++) {
        if (
          productArray[i].name.toLowerCase().search(productsearch) != -1 &&
          productArray[i].type.toLowerCase() == typesearch.toLowerCase() &&
          parseInt(productArray[i].price) >= parseInt(pricefrom) &&
          parseInt(productArray[i].price) <= parseInt(priceto)
        ) {
          tmparr.push(productArray[i]);
      ...
      if (index >= start && index < end) {
        s += `<li id="card">
              <div class="product-item">
                    ...
                    <button class="icon-btn" id="atc" onClick="addToCart(${
                      item.id
                    })"><i class="fa-sharp fa-solid fa-cart-plus"></i></button>
                    <button class="buy-now" id="bn${index}" onclick="showProductInfo('${
          item.id
        }')">MUA NGAY</button>
              </div>
            </li>`;
...
function addCart(index) {
  let productArray = localStorage.getItem("product")
...
function upSL(index) {
  let carts = localStorage.getItem("carts")
    ? JSON.parse(localStorage.getItem("carts"))
    : [];
  document.getElementsByClassName("sl2")[index].value =
    Number.parseInt(carts[index].sl, 10) + 1;
  carts[index] = {
    id: carts[index].id,
...
function downSL(index) {
  ...
  if (
    ...
    document.getElementsByClassName("sl2")[index].value = Number.parseInt(
      carts[index].sl,
      10
    );
  }
...
function checkCart() {
  let customer = localStorage.getItem("userlogin")
